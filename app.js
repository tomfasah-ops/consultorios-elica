...
async function loginProfesional(e){
  e.preventDefault();
  const usuario=$('profLoginUsuario').value.trim().toLowerCase();
  const clave=$('profLoginClave').value.trim();
  ...
  if(error){
    show('profLoginMsg',error.message,'error');
    return;
  }

  if(!data || !data.length){
    show('profLoginMsg','Usuario o contraseña incorrectos. Consultá con administración.','error');
    return;
  }

  const prof=data[0];
  localStorage.setItem('elica_profesional_ok','ok');
  localStorage.setItem('elica_profesional_nombre',prof.nombre||'');
  localStorage.setItem('elica_profesional_especialidad',prof.especialidad||'');
...
async function cargarAgendaProfesional(){
  ...
  if($('profAgendaFecha')) $('profAgendaFecha').value=fecha;
  ...
  if(error){
    show('profAgendaMsg',error.message,'error');
    return;
  }
...
function abrirTurnoProfesional(id){
  ...
  $('modalContenido').innerHTML=`
    <div class="modal-grid">
      <p><b>Fecha:</b> ${esc(t.fecha)}</p>
      <p><b>Hora:</b> ${esc(String(t.hora).slice(0,5))} hs</p>
      <p><b>Profesional:</b> ${esc(t.profesional)}</p>
      <p><b>Especialidad:</b> ${esc(t.especialidad)}</p>
      <p><b>Teléfono:</b> ${esc(t.telefono)}</p>
      <p><b>DNI:</b> ${esc(t.dni)}</p>
      <p><b>Obra social:</b> ${esc(t.obra_social)}</p>
      <p><b>Estado:</b> ${esc(t.estado||'confirmado')}</p>
    </div>
    <hr>
    <p><b>Motivo:</b><br>${esc(t.motivo_consulta||'Sin motivo cargado')}</p>
    <div class="modal-actions">
      <button class="btn ok" onclick="abrirHistoriaDesdeTurno(${t.id})">Abrir historia clínica</button>
      <button class="btn secondary" onclick="marcarTurnoAtendido(${t.id})">Marcar como atendido</button>
      <button class="btn secondary" onclick="cerrarModal()">Cerrar</button>
    </div>
    <div id="historiaAgendaBox"></div>`;
...
async function marcarTurnoAtendido(id){
  ...
  cerrarModal();
  show('profAgendaMsg','Paciente marcado como atendido.','success');
  await cargarAgendaProfesional();
}
...
async function crearProfesional(e){
  ...
  if($('profUsuario')) row.usuario=$('profUsuario').value.trim().toLowerCase();
  if($('profClave')) row.clave=$('profClave').value.trim();

  const {error}=await sb.from('profesionales').insert(row);

  if(error){
    show('profMsg',error.message,'error');
  } else {
    show('profMsg','Profesional guardado. Usuario de acceso creado correctamente.','success');
    e.target.reset();
    await loadConfig();
  }
}
